...
struct liConInfo {
	...
	liStreamSource *req_source;
	liStreamDrain *resp_drain;
	liCQLimit *cqlimit_in, *cqlimit_out;
...
struct liVRequest {
	...
	GPtrArray *filters;
	liStreamSource *filters_in_last, *filters_out_last;
	liStreamDrain *filters_in_first, *filters_out_first;

	liFilterBufferOnDiskState in_buffer_state;

	liPlugin *backend;
	liStreamSource *backend_source;
	liStreamDrain *backend_drain;

	liStreamSource direct_source;
	liStreamDrain direct_drain;
...
LI_API void li_vrequest_indirect_connect(liVRequest *vr, liStreamDrain *backend_drain);
/* received all response headers/status code - call once from your indirect handler */
LI_API void li_vrequest_indirect_response(liVRequest *vr, liStreamSource *backend_source);
